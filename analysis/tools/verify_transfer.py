...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Also require the optional 2017-2025 Sentinel-2 visual exports.",
    )
    args = parser.parse_args()
    root = args.project_root.resolve()
    issues: list[str] = []

    raster_dir = root / "data" / "raw" / "embedding_metrics"
    core = [
    ...
    phase2 = root / "data" / "processed" / "sampling"
    ...
    dea_sample = root / "data" / "processed" / "dea_sample"
    check(
        dea_sample / "basscoast_phase3_dea_long.csv",
        issues,
        "sampled annual DEA history",
    )
    check(
        dea_sample / "basscoast_phase3_dea_enriched_points.csv",
        issues,
        "DEA-enriched sampled points",
    )

    phase9 = root / "data" / "processed" / "map_grid"
    ...
    phase10 = root / "data" / "processed" / "region_context"
    ...
    if args.include_optional_imagery:
        sentinel = root / "data" / "raw" / "sentinel2_annual"
    ...
    if issues:
        ...
        for issue in issues:
            print(f"- {issue}")
        return 1
    scope = "core data and optional imagery" if args.include_optional_imagery else "core data"
    print(f"Transfer check passed: all expected {scope} files are present.")
    return 0
